...
const SETUP_TIMEOUT_MS = Number(process.env.RETHINK_SETUP_TIMEOUT_MS ?? '20000')

function timeoutError() {
    return new Error(`Timed out waiting for ${host}:5500 after ${SETUP_TIMEOUT_MS}ms`)
}
...
async function request(xml: string) {
    const socket = await new Promise<tls.TLSSocket>((resolve, reject) => {
        ...
        socket.setTimeout(SETUP_TIMEOUT_MS, () => socket.destroy(timeoutError()))
...
function thinq2Setup() {
    ...
    // keep the base64 lines at column 0 — no leading whitespace inside the
    // PEM: newer CLIP firmwares reject indented keys (encryptRes:ffff and an
    // endless /route/certificate loop). Aligned with upstream commit 7c52739.
                ...
                if (json.cmd === 'getDeviceInfo')
                    socket.write(
                        JSON.stringify({
                            ...
                            cmd: 'setCertInfo',
                            data: {
                                // svcphase is normally OP. QA/ST makes the module resolve QA cloud hostnames,
                                // which is not useful for the local Home Assistant bridge.
