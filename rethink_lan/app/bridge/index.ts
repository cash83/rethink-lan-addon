...
export class Bridge extends TypedEmitter<BridgeEvents> {
    ...
    #start(dev: AnyDevice) {
        // Sospeso a mano: resta registrato ma non si riattacca da solo quando
        // l'elettrodomestico torna online.
        if (this.state.isSuspended(dev.id)) return
    ...
    // Vero quando c'e' un certificato salvato, cioe' quando il ponte si puo'
    // riattaccare senza rifare tutta la registrazione al cloud.
    isRegistered(id: string) {
        return !!this.state.getDeviceState(id)
    }

    isSuspended(id: string) {
        return this.state.isSuspended(id)
    }

    // Stacca dal cloud senza toccare il certificato: l'opposto di disable(),
    // che invece lo cancella e obbliga a rifare la registrazione.
    suspend(id: string) {
        this.state.setSuspended(id, true)
        this.#stop(id)
    }

    resume(id: string) {
        this.state.setSuspended(id, false)
        const dev = this.manager.allDevices[id]
        if (!dev) return false
        if (this.bridgedDevices.has(id)) return true
        this.#start(dev)
        return this.bridgedDevices.has(id)
    }
