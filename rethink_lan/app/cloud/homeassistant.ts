...
// Quel poco del ponte al cloud LG che serve agli apparecchi per esporre un
// interruttore in Home Assistant. Volutamente minimo: il ponte vero sta in
// bridge/, qui passa solo il filo.
...
export class Connection extends TypedEmitter<ConnectionEvents> {
    ...
    publishConfig(id: string, config: DeviceDiscovery) {
        // Cloud-style availability: tie entities only to the add-on's own
        // availability topic, so a sleeping appliance keeps showing its
        // retained last state instead of going unavailable.
        if (this.config.cloud_style_availability) {
            config = { ...config, availability: [{ topic: '$rethink/availability' }], availability_mode: 'all' }
        }
        ...
        // Retained so the entities survive rethink restarts: HA re-reads the
        // config on subscribe and the availability topics convey the offline
        // state until the appliance reconnects.
