...
type DeviceEvents = {
    ...
    close: () => void
    // Declared here only to keep the thinq1/thinq2 event maps identical: code
    // that holds an `AnyDevice` (the union of both) can only call .on()/.emit()
    // when both sides expose the same event names. Only thinq2 ever emits it.
    link: (info: { rssi?: number }) => void
}
