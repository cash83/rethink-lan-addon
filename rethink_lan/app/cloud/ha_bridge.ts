...
const t2deviceTypes: Record<string, T2Factory> = {
    ...
    ['RC90U2_WW']: RC90U2_WW, // LG RC90U2 dryer, preliminary diagnostics-only support
}
...
class Bridge {
    ...
    newDevice(thinqdev: AnyDevice) {
        ...
        const modelIds = [meta.modelId, meta.modelName]
            .filter((modelId): modelId is string => !!modelId)
            .map((modelId) => modelId.trim())

        if (thinqdev.platform === 'thinq1') {
            const devclass = modelIds.map((modelId) => t1deviceTypes[modelId]).find(Boolean)
            if (devclass) hadevice = new devclass(this.HA, thinqdev, meta)
        } else if (thinqdev.platform === 'thinq2') {
            const devclass = modelIds.map((modelId) => t2deviceTypes[modelId]).find(Boolean)
            if (devclass) hadevice = new devclass(this.HA, thinqdev, meta)
        }

        if (!hadevice) {
            console.warn(
                `${thinqdev.platform} device type unknown modelId=${JSON.stringify(meta.modelId)} modelName=${JSON.stringify(meta.modelName)}`,
            )
            return
        }
